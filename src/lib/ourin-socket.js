...
const { addExifToWebp, DEFAULT_METADATA } = require('./ourin-exif');
const { getFFmpeg } = require('./ourin-ffmpeg-path');
const ffmpeg = getFFmpeg();
const { config } = require('./../../config');
const mime = require('mime-types');
const { getProfilePicture, getProfileBuffer } = require('./ourin-profile-picture');
...
function extendSocket(sock) {
    ...
    sock.getName = async (jid, groupJid = null) => {
        ...
        const { isLid, isLidConverted, resolveAnyLidToJid, getCachedJid } = require('./ourin-lid');
    ...
    sock.getNameFromParticipants = (jid, participants = []) => {
        ...
        const { isLid, isLidConverted, resolveAnyLidToJid } = require('./ourin-lid');
